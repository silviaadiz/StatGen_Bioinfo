...
### ⏩**Imputados con TOPMed e con SNP-ID recuperado.**
...
1. **Imputados fóra de TOPMed e con rs noutro arquivo.** En caso de que teñamos os *rsid* nos *sumstats*, poderíamos facer algo así:
...
```bash
OUT_NAME="OUT_NAME"
# OUT_NAME sería o que usamos no script de execución de PRS-CS

cat ${OUT_NAME}_pst_eff_a1_b0.5_phiauto_chr* > scores_PRScs_${OUT_NA}_allchr.txt
...
### 01. Recuperamos os SNP-ID (en caso de necesitalo)
...
Ademais, recordemos que non todas as variantes tiñan un equivalente tipo *rsid*: debemos excluílas dos arquivos binarios de `plink`. Se ben a función `--score` só usa os SNPs do arquivo de `scores`, imos máis seguras extraendo os SNPs que se usan no PRS e facendo un único binario con todos os cromosomas (explicado máis adiante).
...
🚩Este paso saltaríamolo se os nosos imputados están con rsid. 
...
O script `2_convert_rsid_to_SNP_opt.R` fai isto de forma automática. 
...
Igualmente, en `--score`  indicámoslle de onde collemos (nesta orde) o SNPID/rs, o alelo de efecto e o score. Se partimos da nosa pipeline de imputación + pipeline de PRS-CS(x), o arquivo score que se crea terá esa info nas columnas 2 5 7. En caso de que non teñamos o SNPID, por exemplo, probablemente serían as columnas 2 4 6 (Ver apartado ⏩**Imputados en TOPMed pero mantendo o rs**)**.**
...
### ⏩**Imputados en TOPMed pero mantendo o rs**

Tamén existe a posibilidade de que **teñamos os imputados co `rsid`** porque a última versión de TOPmed exporta os rsid en vez dos SNP-ID e non modificáramos nada en PLINK. Nese caso, faríamos o seguinte:

```bash
OUT_PREFIX="name.imputed"

cat /mnt/lustre/scratch/nlsas/home/usc/gb/sdd/EB25/PRS_Docherty/04.Sens_analysis_GIULIA/new_GIU_imputado08_mon_chr*.bim > bim_rs_${OUT_PREFIX_BIM}_allchr.bim
```

<aside>
⚠️

CAMBIOS NA PIPELINE SE MANTEMOS OS RS DE TOPMED: 

1. Eliminar os scripts de conversión a rs.
2. Engadir isto ao principio de `Apply_score.sh` :
...
# Neste caso, en config_applyscore.sh, os ${SCORE_FILE} e ${SNPS_EXTRACT} non se refiren a ningún arquivo xa existente, senón que se refire ao nome que lle queiramos poñer a estes arquivos.
```

1. Cambiar a PLINK 2 no paso 2 de `Apply_score.sh` e engadir a flag `--rm-dup exclude all` para evitar problemas (os que levan “.” como rs PLINK interprétaos como +3 alelos). Pódese usar a flag `--set-missing-var-ids` pero nalgún caso de variantes duplicadas cos mesmos alelos (por exemplo, unha deleción e unha inserción) tamén protesta.
    1. Nota: hai que poñer ben as rutas de PLINK 2 (substiuír pola ruta onde estea gardado o executable).
    

4. Cambiar, no paso 5 de `Apply_score.sh` , `--score 2 4 6`. Isto é moi importante, porque o alelo de efecto é A1 e aquí temos as columnas que saen con PRS-CS (a orde, por se hai que modificalo, é `SNP EA BETA`).

</aside>

Código con correccións:
